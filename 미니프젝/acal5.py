...
class AttendanceGraph(QWidget):
    ...
    def update_graph(self, daily_counts):
        ...
        # ✅ 선 그래프 그리기
        ax.plot(x_indexes, p_values, marker='o', linestyle='-', color='darkblue', label='출석')
        ax.plot(x_indexes, l_values, marker='s', linestyle='--', color='darkgreen', label='지각')
        ax.plot(x_indexes, a_values, marker='x', linestyle='-.', color='darkred', label='결석')
